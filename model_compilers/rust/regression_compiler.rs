...
fn predict(X: &Array2<f64>, model: &SymbolicModel) -> Array1<f64> {
    let X_transformed = evaluate_basis_functions(X, &model.basis_functions, model.n_features);
    let coeffs = Array1::from(model.coefficients.clone());
    X_transformed.dot(&coeffs)
}

fn main() {
    let file = File::open("outputs/california_housing_model.json").unwrap();
    let reader = BufReader::new(file);
    let symbolic_model: SymbolicModel = serde_json::from_reader(reader).unwrap();

    let X = Array2::<f64>::random((10, symbolic_model.n_features), rand::distributions::Standard);
